...
(defun init-dict (env)
  (add-word 's '(format t "~s" (env-stack env)) env t)
  ...
  (add-word 'code '(print (symbol-plist (find-word (intern (string-upcase (stack-pop env))) env))) env t)
  (add-word '_ '(print (stack-pop env)) env t)
  (add-word '= '(stack-push (= (stack-pop env) (stack-pop env)) env) env t)
  (add-word 'clear '(setf (env-stack env) nil) env t)
  ;; (add-word 'if '(let (then else acc tmp)
  ;; 		  (loop while (not (or (eq (setf tmp (forth-read s)) 'else)
  ;; 				       (eq (setf tmp (forth-read s)) 'then)))
  ;; 		     do (push tmp then))
  ;; 		  (loop while (not (eq (setf tmp (forth-read s)) 'then))
  ;; 		     do (push tmp else))))
